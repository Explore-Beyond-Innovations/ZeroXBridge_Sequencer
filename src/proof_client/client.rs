...
use crate::{
    ...
    proof_client::{
        input_generator::generate_cairo1_inputs,
        proof_generator::run_scarb_build,
    },
};
...
impl ProofClientService {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        let mut interval = time::interval(Duration::from_secs(
            self.config.queue.process_interval_sec
        ));
    ...
    pub async fn process_pending_deposits(&self) -> Result<()> {
        ...
        let pending_deposits = self.fetch_pending_proof_deposits().await
        ...
        info!("Found {} deposits pending proof generation", pending_deposits.len());
        ...
        for deposit in pending_deposits {
            match self.process_single_deposit(&deposit).await {
                ...
                Err(e) => {
                    error!(
                        "Critical error processing deposit ID {}: {}", 
                        deposit.id, e
                    );
    ...
    pub async fn fetch_merkle_proof_data(&self, commitment_hash: &str) -> Result<DepositHashAppended, ProofGenerationError> {
        use sqlx::Row;
        ...
        match row {
            ...
            None => Err(ProofGenerationError::MerkleProofNotFound(commitment_hash.to_string()))
        }
    }
        ...
        let commitment_u64 = if commitment_bytes.len() >= 8 {
            u64::from_be_bytes(
                commitment_bytes[commitment_bytes.len() - 8..]
                    .try_into()
                    .map_err(|e| ProofGenerationError::InvalidProofData(format!("Invalid commitment hash: {}", e)))?
            )
        } else {
        ...
        let _root_u64 = if merkle_data.root_hash.len() >= 8 {
            u64::from_be_bytes(
                merkle_data.root_hash[merkle_data.root_hash.len() - 8..]
                    .try_into()
                    .map_err(|e| ProofGenerationError::InvalidProofData(format!("Invalid root hash: {}", e)))?
            )
        } else {
            return Err(ProofGenerationError::InvalidProofData(
    ...
    async fn process_single_deposit(&self, deposit: &Deposit) -> Result<ProofGenerationResult> {
        ...
        let merkle_data = self.fetch_merkle_proof_data(&deposit.commitment_hash).await?;
        ...
        self.generate_cairo_inputs(&input_array, &temp_dir).await
            .context("Failed to generate Cairo1 inputs")?;

        // Step 4: Run Scarb build
        let sierra_path = self.run_scarb_build().await
            .context("Failed to run Scarb build")?;

        // Step 5: Run Stone pipeline
        let artifacts = self.run_stone_pipeline(&sierra_path, &input_array, &temp_dir).await
            .context("Failed to run Stone pipeline")?;

        // Step 6: Persist artifacts
        let artifacts_path = self.persist_artifacts(&artifacts, deposit.id).await
            .context("Failed to persist artifacts")?;

        // Step 7: Update deposit status
        self.update_deposit_status(deposit.id, "READY_FOR_RELAY").await
    ...
    async fn fetch_pending_proof_deposits(&self) -> Result<Vec<Deposit>, sqlx::Error> {
        ...
        let deposits = rows.into_iter().map(|row| Deposit {
            id: row.get("id"),
    ...
    async fn run_scarb_build(&self) -> Result<PathBuf, ProofGenerationError> {
        let sierra_path = run_scarb_build(
            self.proof_generator_path.to_str()
                .ok_or_else(|| ProofGenerationError::InvalidProofData("Invalid proof generator path".to_string()))?
        )
...
mod tests {
    use super::*;
    use crate::{
        config::{AppConfig, QueueConfig, DatabaseConfig, EthereumConfig, StarknetConfig, 
                ServerConfig, ContractConfig, Contracts, RelayerConfig, MerkleConfig, 
                LoggingConfig, OracleConfig, HerodotusConfig},
    ...
    async fn test_proof_client_service_creation() {
        ...
        assert!(service.proof_generator_path.to_str().unwrap().contains("proof-generator"));
        assert!(service.prover_params_path.to_str().unwrap().contains("prover_params.json"));
        assert!(service.prover_config_path.to_str().unwrap().contains("prover_config.json"));
    }
    ...
    async fn test_build_input_array_valid_data() {
        ...
        let db_pool = PgPool::connect_lazy("postgresql://postgres:postgres@localhost:5434/zeroxdb").unwrap();
        let service = ProofClientService::new(db_pool, config);
    ...
    async fn test_build_input_array_short_commitment_hash() {
        ...
        let merkle_data = create_test_deposit_hash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef");
    ...
    fn test_proof_generation_error_display() {
        ...
        assert!(invalid_data_error.to_string().contains("Invalid proof data"));
    ...
    fn test_hex_decoding_with_0x_prefix() {
        ...
        let hash_without_prefix = "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef";
